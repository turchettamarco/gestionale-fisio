...
const [filtersExpanded, setFiltersExpanded] = useState(false);
  ...
  // Appuntamenti imminenti (auto-aggiornati ogni minuto tramite currentTime)
  // - mostra solo quelli non ancora terminati
  // - mantiene l'ordinamento per orario
  const upcomingAppointments = useMemo(() => {
    const now = currentTime;
    return [...todaysAppointments]
      .filter((a) => a?.end instanceof Date ? a.end.getTime() >= now.getTime() : true)
          ...
          {upcomingAppointments.length === 0 ? (
            <div style={{ 
              ...
              <div style={{ fontSize: 11, color: THEME.muted }}>
                Tutti gli appuntamenti di oggi sono finiti (o non ce ne sono)
              </div>
            </div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 12, maxHeight: 5 * 84, overflowY: "auto", paddingRight: 4 }}>
              {upcomingAppointments.map((appointment) => {
                const now = currentTime;
                const isPast = appointment.end < now;
                    ...
                    <div style={{ display: "flex", alignItems: "flex-start", gap: 8, marginLeft: 4 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ 
                          ...
                          <span title={appointment.patient_name}>{appointment.patient_name}</span>
                        </div>
          ...
          <div style={{ marginTop: 20, fontSize: 11, color: THEME.muted, textAlign: "center" }}>
            {todaysAppointments.length > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span>In corso: {todaysAppointments.filter(a => a.start <= currentTime && a.end >= currentTime).length}</span>
                <span>Rimanenti: {upcomingAppointments.length}</span>
              </div>
            )}
          </div>
        </div>
