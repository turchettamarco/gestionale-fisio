...
/* ─── Main ────────────────────────────────────────────────────────────── */
export default function PatientDetailMobileClient({ patientId }: { patientId: string }) {
  ...
  if (!patient) return (
    <div style={{ minHeight: "100vh", background: T.appBg, padding: 20,
      ...
      <Link href="/patients" style={{ color: T.blue, fontWeight: 600, fontSize: 14 }}>
        ← Torna ai pazienti
      </Link>
    </div>
  );
...
function MobilePortalTab({ patient, currentStudio }: {
  ...
  async function sendWA() {
    ...
    if (isMob) {
      // Schema URI nativo: apre app WhatsApp diretto, no api.whatsapp.com
      if (waWindow) waWindow.close();
      const enc = encodeURIComponent(msg);
      window.location.href = `whatsapp://send?phone=${n}&text=${enc}`;
      setTimeout(() => {
        if (document.visibilityState === "visible") {
          window.location.href = `https://wa.me/${n}?text=${enc}`;
        }
      }, 1500);
    } else {
      const waUrl = `https://web.whatsapp.com/send?phone=${n}&text=${encodeURIComponent(msg)}`;
