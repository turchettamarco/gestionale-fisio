...
// (load fiscale/preferenze parziale rimossa: ora copre tutto loadPracticeFull)
  ...
  async function moveService(id: string, direction: "up" | "down") {
    ...
    const results = await Promise.all([
      supabase.from("booking_services").update({ sort_order: newA }).eq("id", a.id).eq("studio_id", currentStudioId),
      supabase.from("booking_services").update({ sort_order: newB }).eq("id", b.id).eq("studio_id", currentStudioId),
    ]);
