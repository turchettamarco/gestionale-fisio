...
export async function compilaModuloOriginale(d: CartellaData): Promise<Blob> {
  ...
  const INK = rgb(0, 0, 0);                 // nero pieno: leggibile anche stampato
  ...
  const scrivi = (
    ...
    if (opt.max) {
      while (size > 7.5 && f.widthOfTextAtSize(s, size) > opt.max) size -= 0.4;
    }
  ...
  const evidenzia = (pageIdx: number, top: number, x0: number, x1: number) => {
    ...
    pg.drawText("X", {
      x: x1 - 10, y: ALTEZZA - top - 9.5, size: 10.5, font: fontB, color: rgb(0, 0, 0),
    });
  };
  ...
  // ═══════════ p3 — Consenso informato ═══════════
  {
    ...
    scrivi(P, C.natoIl[0], C.natoIl[1], itDate(d.data_nascita), { size: 11, max: C.natoIl[2] });
    ...
    scrivi(P, C.tutoreNatoIl[0], C.tutoreNatoIl[1], itDate(d.tutore_nascita), { size: 11, max: C.tutoreNatoIl[2] });
  ...
  if (d.responsabilita) {
    ...
    scrivi(P, C.attivazionePai[0], C.attivazionePai[1], itDate(d.attivazione_pai), { size: 10.5, max: 48 });
    scrivi(P, C.assistito[0], C.assistito[1], `${d.nome} ${d.cognome}`.trim(), { max: C.assistito[2] });
    scrivi(P, C.assistitoNatoIl[0], C.assistitoNatoIl[1], itDate(d.data_nascita), { size: 10.5, max: 48 });
  ...
  // ═══════════ p9 — ADL e IADL ═══════════
  {
    ...
    scrivi(P, H.nascita[0], H.nascita[1], itDate(d.data_nascita), { size: 11, max: H.nascita[2] });
    scrivi(P, H.valutaz[0], H.valutaz[1], itDate(d.data_valutazione), { size: 11, max: H.valutaz[2] });
    ...
    scrivi(P, TOTALI.adl[0], TOTALI.adl[1], String(scoreBlock(ADL, d.risposte)), { bold: true, size: 14 });
    scrivi(P, TOTALI.iadl[0], TOTALI.iadl[1], String(scoreBlock(IADL, d.risposte)), { bold: true, size: 14 });
  }
  ...
  // ═══════════ p11 — MMSE ═══════════
  {
    ...
    scrivi(P, TOTALI.mmseGrezzo[0], TOTALI.mmseGrezzo[1], String(scoreMmse(d.risposte)), { bold: true, size: 13 });
    if (d.mmse_aggiustato) {
      scrivi(P, TOTALI.mmseAggiust[0], TOTALI.mmseAggiust[1], d.mmse_aggiustato, { bold: true, size: 13 });
    }
  }
  ...
  // ═══════════ p13 — Tinetti ═══════════
  {
    ...
    scrivi(P, TOTALI.tinettiEq[0], TOTALI.tinettiEq[1], String(eq), { bold: true, size: 13 });
    scrivi(P, TOTALI.tinettiAnd[0], TOTALI.tinettiAnd[1], String(an), { bold: true, size: 13 });
